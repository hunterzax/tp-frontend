...
import { formatDate, formatDateNoTime, formatNumber, formatNumberFourDecimal, formatNumberThreeDecimal, handleDownloadPDFTsoView, keepMaxSeqByVersion, underDevelopment } from '@/utils/generalFormatter';
...
const TableVersion: React.FC<TableProps> = ({
    tableData,
    isLoading,
    columnVisibility,
    dataTableHeader,
    ...
    useEffect(() => {
        ...
        switch (WhichOpenDocument) {
            ...
            case 'document_7':
                if (modeOpenDocument == 'version_edit') {
                    ...
                    text_header = 'ประวัติเอกสารคำสั่งเพิ่ม/ลดปริมาณก๊าซ (Doc 7)'
                }
                setDocumentNoText(text_header)
                break;
        }
        ...
        if (tableData && tableData.length > 0) {
            // Shipper Edit ข้อมูลได้รับเกิน https://app.clickup.com/t/86eum0paq
            if (userDT?.account_manage?.[0]?.user_type_id == 3) {
                let res_:any = keepMaxSeqByVersion(tableData)
                setSortedData(res_)
                setMainData(res_)
                setDataOriginal(res_)
            } else {
                setSortedData(tableData)
                setMainData(tableData)
                setDataOriginal(tableData)
            }
        } else {
            setSortedData([]);
            setMainData([]);
    ...
    const getDataDocument = async (id: String, doc_no: any) => {
        try {
            const res_document_data = await getService(`/master/event/ofo/doc${doc_no}/version/doc/${id}`)
    ...
    const openDocument = async (document: String, mode?: String, data?: any) => {
        if (document == 'document_4') {
            const data_doc_4 = await getDataDocument(data.id, 4)
            setDataOpenDocument(data_doc_4)
        } else if (document == 'document_7') {
            const data_doc_7 = await getDataDocument(data.id, 7)
                ...
                <thead className="text-xs text-[#ffffff] bg-[#1473A1] sticky top-0 z-10">
                    <tr className="h-9">
                        ...
                        <th scope="col"
                            ...
                            onClick={() => handleSort("version_text", sortState, setSortState, setSortedData, dataOriginal)}
                        >
                            {`Version`}
                        ...
                        <th scope="col"
                            ...
                            onClick={() => handleSort("create_date", sortState, setSortState, setSortedData, dataOriginal)}
                        >
                            {`Action Date`}
                        ...
                        <th scope="col"
                            ...
                            onClick={() => handleSort("create_by_account.first_name", sortState, setSortState, setSortedData, dataOriginal)}
                        >
                            {`Edited By`}
                                                ...
                                                userDT?.account_manage?.[0]?.user_type_id == 3 ? // SHIPPER
                                                    (userDT?.account_manage?.[0]?.user_type_id == 3 && row?.event_doc_status_id == 5) || modeOpenDocument == 'view' ?
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            onClick={() => openDocument('document_7', 'view', row)}
                                                        >
