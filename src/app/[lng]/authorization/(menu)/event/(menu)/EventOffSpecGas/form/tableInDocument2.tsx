...
import { formatDate, handleDownloadPDFTsoView } from '@/utils/generalFormatter';
...
const TableDocument2: React.FC<TableProps> = ({ tableData, isLoading, columnVisibility, dataOpenDocument }) => {
                    ...
                    {sortedData && sortedData.map((row: any, index: any) => {
                        return (
                            ...
                            >
                                ...
                                <td className="px-4 py-1 text-[#464255] ">{dataOpenDocument ? dataOpenDocument?.event_runnumber?.event_nember : ''}</td>
                                ...
                                <td className="px-4 py-1 text-center">
                                    {(row?.event_doc_status_id === 3 || row?.event_doc_status_id === 4 || row?.event_doc_status_id === 5) && (
                                        <div
                                            className="mx-auto flex w-[120px] justify-center rounded-full px-1 py-2 text-[#464255]"
                                            style={{ backgroundColor: row?.event_doc_status?.color }}
                                        >
                                            {row?.event_doc_status?.name ?? ''}
                                        </div>
                                    )}
                                </td>
                                ...
                                <td className="px-4 py-1 text-center">
                                    ...
                                    >
                                        <button type="button" className={class_btn_} disabled={false}
                                            onClick={() => handleDownloadPDFTsoView('doc2', row?.id, row?.update_by, row?.group_id)}
                                        >
