...
const ClientPage: React.FC<ClientProps> = () => {
    ...
    const fetchData = async (offset?: any, limit?: any) => {
        try {
            ...
            const res_main_data = await getService(`/master/event/offspec-gas?eventCode=&eventDateFrom=${eventDateFrom}&eventDateTo=${eventDateTo}&EventStatus=&offset=${offset}&limit=${limit}`);
        ...
        switch (WhichOpenDocument) {
            case 'document_1':
                if (modeOpenDocument == 'create') {
                    ...
                    if (res_create?.response?.data?.status === 400 || res_create?.response?.data?.status === 500 || res_create?.response?.data?.statusCode === 500) {
            ...
            case 'document_2':
                if (modeOpenDocument == 'create') {
                    ...
                    if (userDT?.account_manage?.[0]?.user_type_id !== 3) {
                        ...
                        if (res_edit_tso?.response?.data?.status === 400 || res_edit_tso?.response?.data?.status === 500 || res_edit_tso?.response?.data?.statusCode === 500) {
                            setModalErrorMsg(res_edit_tso?.response?.data?.error ? res_edit_tso?.response?.data?.error : "Something went wrong.");
                            setModalErrorOpen(true)
                        } else {
                            // setFormOpen(false);
                            await fetchData(0, 10);
                        ...
                        if (res_edit_shipper?.response?.data?.status === 400 || res_edit_shipper?.response?.data?.status === 500 || res_edit_shipper?.response?.data?.statusCode === 500) {
            ...
            case 'document_3':
                ...
                if (modeOpenDocument == 'create') {
                    ...
                    if (res_create_doc_3?.response?.data?.status === 400 || res_create_doc_3?.response?.data?.status === 500 || res_create_doc_3?.response?.data?.statusCode === 500) {
                        setModalErrorMsg(res_create_doc_3?.response?.data?.error ? res_create_doc_3?.response?.data?.error : "Something went wrong.");
                        setModalErrorOpen(true)
                    } else {
                        // setFormOpen(false);
                        await fetchData(0, 10);
                    ...
                    if (userDT?.account_manage?.[0]?.user_type_id !== 3) {
                        ...
                        if (res_shipper_acknow?.response?.data?.status === 400 || res_shipper_acknow?.response?.data?.status === 500 || res_shipper_acknow?.response?.data?.statusCode === 500) {
                            setModalErrorMsg(res_shipper_acknow?.response?.data?.error ? res_shipper_acknow?.response?.data?.error : "Something went wrong.");
                            setModalErrorOpen(true)
                        } else {
                            // setFormOpen(false);
                            await fetchData(0, 10);
                            setModalSuccessMsg('Event Document been acknowledged.')
                            setModalSuccessOpen(true);
                        }
                    }
                }
