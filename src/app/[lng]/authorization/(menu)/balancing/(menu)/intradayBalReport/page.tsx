...
const ClientPage: React.FC<ClientProps> = () => {
    ...
    const getPermission = () => {
        try {
            ...
            if (user_permission?.role_config) {
                ...
                const permission = findRoleConfigByMenuName('Intraday Balancing Report', userDT)
    ...
    const [srchStartDate, setSrchStartDate] = useState<Date | null>(() => { return dayjs().toDate(); }); // v2.0.46 default Gas day ต้องเป็นวันปัจจุบัน https://app.clickup.com/t/86etym1t4
    ...
    // Filter Timestamp ต้องกรองตาม Gas Hour From , Gas Hour To
    useEffect(() => {
    ...
    const handleFieldSearch = async () => {
        ...
        const from = (srchGasHourFrom !== undefined && srchGasHourFrom !== '') ? hourStringToInt(srchGasHourFrom) : 0;
        const to = (srchGasHourTo !== undefined && srchGasHourTo !== '') ? hourStringToInt(srchGasHourTo) : 24;
        ...
        const body_main = {
            ...
            "latest_hourly_version": userDT?.account_manage?.[0]?.user_type_id !== 3 ? watch('filter_last_hourly_version') : true, // Shipper จะเห็นเฉพาะ version ล่าสุดที่โดน public ของแต่ละ Gas Hour https://app.clickup.com/t/86eujrgh9
        ...
        // let res_: any = new_data_with_actual_planning // ---> mock data ใหม่ เบิ้ลสอง row
            ...
            let res_filtered = res_?.map((item: any) => {
                ...
                const filteredShipperData = item.shipperData?.map((sd: any) => {
                    ...
                    const filteredContracts = sd?.contractData?.filter((cd: any) => {
                        ...
                        const matchGasHour = fromMinutes !== null && toMinutes !== null ? gasHourMin >= fromMinutes && gasHourMin <= toMinutes : true;
    ...
    const handleSearch = (query: string) => {
        ...
        debounceRef.current = setTimeout(() => {
            ...
            //     const totalAllMatch2 = keyValues.some((key) => {
            //         const val = entry?.totalAllPlanning?.[key];
            //         return (
            //             normalize(formatNumberFourDecimal(val).includes(queryLower)) ||
            //             normalize(formatNumberFourDecimalNoComma(val).includes(queryLower))
            //         );
            //     });
            //     const totalAllMatch2Actual = keyValues.some((key) => {
            //         const val = entry?.totalAllActual?.[key];
            //         return (
            //             normalize(formatNumberFourDecimal(val)).includes(queryLower) ||
            //             normalize(formatNumberFourDecimalNoComma(val)).includes(queryLower)
            //         )
            ...
            //         const totalShipperMatch2 = keyValues.some((key) => {
            //             const val = shipperGroup?.totalShipperPlanning?.[key];
            //             return (
            //                 normalize(formatNumberFourDecimal(val)).includes(queryLower) ||
            //                 normalize(formatNumberFourDecimalNoComma(val)).includes(queryLower)
            //             );
            ...
            //         const totalShipperMatch2Actual = keyValues.some((key) => {
            //             const val = shipperGroup?.totalShipperActual?.[key];
            //             return (
            //                 normalize(formatNumberFourDecimal(val)).includes(queryLower) ||
            //                 normalize(formatNumberFourDecimalNoComma(val)).includes(queryLower)
            //             );
            ...
            //             // const valueMatch = keyValues.some((key) => normalize(formatNumberFourDecimal(planning[key])).includes(queryLower));
            //             // const valueMatchActual = keyValues.some((key) => normalize(formatNumberFourDecimal(actual[key])).includes(queryLower));

            //             const valueMatch = keyValues.some((key) => {
            //                 return (
            //                     normalize(formatNumberFourDecimal(planning[key])).includes(queryLower) ||
            //                     normalize(formatNumberFourDecimalNoComma(planning[key])).includes(queryLower)
            //                 )
            //             });
            //             const valueMatchActual = keyValues.some((key) => {
            //                 return (
            //                     normalize(formatNumberFourDecimal(actual[key])).includes(queryLower) ||
            //                     normalize(formatNumberFourDecimalNoComma(actual[key])).includes(queryLower)
            //                 )
            //             });
            ...
            const filtered = filter_plan_actual?.map((entry: any) => {
                ...
                const filteredShipperData = entry.shipperData?.map((shipperGroup: any) => { // --> row blue
                    const totalShipperMatch = normalize(shipperGroup?.totalShipperPlanning?.gas_day).includes(queryLower);
                    const totalShipperMatchActual = normalize(shipperGroup?.totalShipperActual?.gas_day).includes(queryLower);
                    ...
                    const filteredContracts = shipperGroup.contractData?.filter((contract: any) => { // --> row white
                        const planning = contract.valueContractPlanning || {};
                        const actual = contract.valueContractActual || {};
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // const response: any = new_data_with_actual_planning
    ...
    const initialColumns: any = [
        ...
        { key: 'acc_imb_inventory_mmbtud', label: 'Acc. IMB. Inventory (MMBTU/D)', visible: false, parent_id: 'summary_pane' },
        ...
        // v2.0.46 ขาด column Acc. Imbalance Inventory ทั้ง TSO และ Shipper https://app.clickup.com/t/86etym1r5
        // sub of Summary Pane --> Acc. IMB. Inventory (MMBTU/D)
        { key: 'east_acc_imb_inventory_mmbtud', label: 'East', visible: false, parent_id: 'acc_imb_inventory_mmbtud' },
        { key: 'west_acc_imb_inventory_mmbtud', label: 'West', visible: false, parent_id: 'acc_imb_inventory_mmbtud' },
            ...
            <div className="border-[#DFE4EA] border-[1px] p-4 rounded-xl  flex flex-col sm:flex-row gap-2">
                <aside className="flex flex-wrap sm:flex-row gap-2 w-full">
                    ...
                    <DatePickaSearch
                        ...
                        isDefaultToday={true} // v2.0.46 default Gas day ต้องเป็นวันปัจจุบัน https://app.clickup.com/t/86etym1t4
