...
const ModalComment: React.FC<FormExampleProps> = ({
    ...
    useEffect(() => {
        if (data) {
            ...
            const find_shipper = dataShipperGroup?.find((itemx: any) => itemx?.id_name == dataRow?.shipperName)
            setShipperData(find_shipper)

            // const filtered_comment_tso = data?.filter((item: any) => item?.create_by_account?.userType?.id !== 3);
            // const filtered_comment_shipper = data?.filter((item: any) => item?.create_by_account?.userType?.id == 3);

            const filtered_comment_tso = data?.filter((item: any) => item?.comment_tap_user_type !== 3);  // 2 tab tso, 3 tab shipper
            const filtered_comment_shipper = data?.filter((item: any) => item?.comment_tap_user_type == 3);  // 2 tab tso, 3 tab shipper

            setCommentsShipper(filtered_comment_shipper)
            setCommentsTso(filtered_comment_tso)
        }
    }, [data])
    ...
    }, [tabMain, commentsShipper, commentsTso])
    ...
    const handleSendComment = async () => {
        ...
        let data_post = {
            "comment": commentText,
            "comment_tap_user_type": tabMain == 0 ? 3 : 2 // 2 tab tso, 3 tab shipper
        }

        // หรือคุณจะบอกว่าผมไม่เฟี้ยวอะ
        const res_comment = await putService(`/master/balancing/instructed-operation-flow-shippers-comment/${dataRow?.id}`, data_post)
        const newComment = {
            comment: commentText,
                    ...
                    <div className="flex flex-col items-center gap-2 p-9">
                        <div className="w-[700px]">
                            ...
                            <div className="mb-4 w-[100%]">
                                ...
                                <div className="grid grid-cols-5 text-sm font-light text-[#58585A]">
                                    <p>{dataRow?.timestamp || ''}</p>
                                    {/* <p>{dataRow?.shipperName || ''}</p> */}
                        ...
                        <div className="w-[700px]">
                            ...
                            >
                                ...
                                {/* userDT?.account_manage?.[0]?.user_type_id == 3 จะเหลือแค่ shipper */}
