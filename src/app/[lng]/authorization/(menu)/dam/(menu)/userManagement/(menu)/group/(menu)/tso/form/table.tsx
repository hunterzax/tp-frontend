...
import { anonymizeEmail, formatDateNoTime, iconButtonClass, maskLastFiveDigits } from '@/utils/generalFormatter';
...
interface TableProps {
  ...
  userPermission?: any;
  isspinLoading?: any;
  setisspinLoading?: any;
}

const TableTsoGroup: React.FC<TableProps> = ({ openEditForm, openViewForm, openDiv, handleActive, openHistoryForm, tableData, isLoading, columnVisibility, istrLoading, settrIsLoading, selectedKey, setselectedKey, userPermission, isspinLoading, setisspinLoading }) => {
              ...
              <tr className="h-9">
                ...
                {columnVisibility.role_default && (
                  <th
                    scope="col"
                    className={`${table_sort_header_style}`}
                    onClick={() => handleSort("role_default[0].role.name", sortState, setSortState, setSortedData, tableData)}
                  >
                    {`Default Role`}
                    {getArrowIcon("role_default[0].role.name")}
                  </th>
                )}
                ...
                {columnVisibility.telephone && (
                  <th
                    scope="col"
                    className={`${table_sort_header_style}`}
                    onClick={() => handleSort("telephone", sortState, setSortState, setSortedData, tableData)}
                  >
                    {`Telephone`}
                ...
                {columnVisibility.email && (
                  <th
                    scope="col"
                    className={`${table_sort_header_style}`}
                    onClick={() => handleSort("email", sortState, setSortState, setSortedData, tableData)}
                  >
                    {`Email`}
                    {getArrowIcon("email")}
                  </th>
