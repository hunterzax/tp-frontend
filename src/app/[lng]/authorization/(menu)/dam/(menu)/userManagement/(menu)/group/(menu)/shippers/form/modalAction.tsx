                    ...
                    >
                        <div className="flex inset-0 items-center justify-center ">
                            <div className="flex flex-col items-center justify-center gap-2 rounded-md w-[700px]">
                                ...
                                >
                                    <h2 className="text-xl font-bold text-[#00ADEF] mb-4 pb-5">{mode === 'create' ? 'Add' : mode === 'edit' ? 'Edit' : mode === 'view' ? 'View' : 'Duplicate'}{` Shipper`}</h2>

                                    <div className="grid grid-cols-2 gap-x-[20px] gap-y-2">
                                        <div className="pb-2">
                                            <label className={labelClass}><span className="text-red-500">*</span>{`Shipper ID`}</label>
                                            <input
                                                type="text"
                                                {...register('id_name', { required: "Enter shipper ID" })}
                                        ...
                                        <div className="pb-2">
                                            <label className={labelClass}><span className="text-red-500">*</span>{`Shipper name`}</label>
                                            <input
                                                type="text"
                                                {...register('name', { required: "Enter shipper name" })}
                                        ...
                                        <div className="pb-2 col-span-2">
                                            <label className={labelClass}><span className="text-red-500">*</span>{`Shipper Company Name`}</label>
                                            <input
                                                type="text"
                                                {...register('company_name', { required: "Enter shipper company name" })}
                                                className={`${inputClass} ${isReadOnly && '!bg-[#EFECEC]'} ${errors.company_name && 'border-red-500'}`}
                                                placeholder="Enter Shipper Company Name"
                                                readOnly={isReadOnly}
                                            />
                                            {errors.company_name && <p className={`${textErrorClass}`}>{errors.company_name.message}</p>}
                                        </div>
                                        ...
                                        <div className="pb-2">
                                            ...
                                            <Select
                                                ...
                                                multiple
                                                // className={`${isReadOnly && '!bg-[#EFECEC]'} ${errors.division && 'border-red-500'}`}
                                                // ${mode === 'view' && '!border-none'}
                                                className={`${selectboxClass} ${isReadOnly && '!bg-[#EFECEC]'} ${errors.division && 'border-red-500'} ${mode == 'view' && '!text-black'}`}
                                                sx={{
                                            ...
                                            >
                                                {
                                                    /* 'Select All' option - Render only if there are items */
                                                    dataDivNotUse?.length > 0 && (
                                        ...
                                        <div className="pb-2 col-span-2">
                                            ...
                                            <input
                                                type="text"
                                                {...register('address')}
                                                className={`${inputClass} ${isReadOnly && '!bg-[#EFECEC]'} ${errors.address && 'border-red-500'}`}
                                                placeholder="Enter Address"
                                                readOnly={isReadOnly}
                                            />

                                            {errors.address && <p className={`${textErrorClass}`}>{errors.address.message}</p>}
                                        </div>

                                        <div className="pb-2">
                                            <label className={labelClass}>{`Bank Account`}</label>
                                            <Select
                                                id='select_shipper_bank_master_id'
                                                IconComponent={(props) => <ExpandMoreIcon {...props} fontSize="medium" />}
                                                {...register('bank_master_id')}
                                                disabled={isReadOnly}
                                                value={watch('bank_master_id') || ''}
                                                className={`${selectboxClass} ${isReadOnly && '!bg-[#EFECEC]'} ${errors.bank_master_id && 'border-red-500'}`}
                                                sx={{
                                                    '.MuiOutlinedInput-notchedOutline': {
                                                        borderColor: errors.bank_master_id ? '#FF0000' : '#DFE4EA',
                                                    },
                                                    '&:hover .MuiOutlinedInput-notchedOutline': {
                                                        borderColor: errors.bank_master_id && !watch("bank_master_id") ? "#FF0000" : "#d2d4d8",
                                                    },
                                                    '&.Mui-focused .MuiOutlinedInput-notchedOutline': {
                                                        borderColor: '#d2d4d8',
                                                    },
                                                ...
                                                onChange={(e) => {
                                                    setValue('bank_master_id', e.target.value);
                                                }}
                                                displayEmpty
                                                renderValue={(value: any) => {
                                                    if (!value) {
                                                        return <Typography color="#9CA3AF" className={isReadOnly ? 'opacity-0' : 'opacity-100'} fontSize={14}>Select Bank Account</Typography>;
                                                    }
                                                    const filterItem: any = dataBank.find((item: any) => item.id === value);
                                                    return (
                                                        <div className={`${itemselectClass} flex items-center`}>
                                                            <img src={filterItem?.url} alt={filterItem?.name} style={{ width: '24px', height: '24px', marginRight: '8px' }} />
                                                            <div className='text-ellipsis whitespace-nowrap overflow-hidden w-[98%]'>{filterItem?.name}</div>
                                                        </div>
                                                    )
                                                }}
                                            >
                                                {dataBank?.slice() // เพื่อไม่เปลี่ยนต้นฉบับ
                                                    .sort((a: any, b: any) => {
                                                        const textA = (a?.name ?? '').toString().toLowerCase();
                                                        const textB = (b?.name ?? '').toString().toLowerCase();
                                                        return textA?.localeCompare(textB);
                                                    })?.map((item: any) => (
                                                        <MenuItem key={item.id} value={item.id}>
                                            ...
                                            {errors.bank_master_id && <p className={`${textErrorClass}`}>{errors.bank_master_id.message}</p>}
                                        </div>
                                        ...
                                        <div className="pb-2 ">
                                            ...
                                            <input
                                                type="text"
                                                {...register('bank_no')}
                                                className={`${inputClass} ${isReadOnly && '!bg-[#EFECEC]'} ${errors.bank_no && 'border-red-500'}`}
                                                placeholder="Enter Account Number"
                                                readOnly={isReadOnly}
                                            />
                                            {errors.bank_no && <p className={`${textErrorClass}`}>{errors.bank_no.message}</p>}
                                        </div>
