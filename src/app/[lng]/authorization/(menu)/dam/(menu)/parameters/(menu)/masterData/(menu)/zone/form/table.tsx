...
const TableZone: React.FC<TableProps> = ({ openEditForm, openViewForm, openHistoryForm, tableData, isLoading, columnVisibility, userPermission }) => {
            ...
            <tbody>
              {sortedData && sortedData?.map((row: any, index: any) => (
                ...
                >
                  ...
                  {/* 
                  {columnVisibility.name && (
                    <td className="px-2 py-1 justify-center text-center">{row?.name && <div className="flex w-[140px] justify-center rounded-full p-1 text-[#464255]" style={{ backgroundColor: row?.color }}>{`${row?.name}`}</div>}</td>
                  )} */}
                  {columnVisibility.name && (
