...
const TableConfigModeZone: React.FC<TableProps> = ({ openEditForm, openViewForm, tableData, isLoading, columnVisibility }) => {
                        ...
                        <thead className="text-xs text-[#ffffff] bg-[#1473A1] sticky top-0 z-10">
                            <tr className="h-9">
                                ...
                                {columnVisibility.mode && (
                                    <th scope="col" className={`${table_sort_header_style}`} onClick={() => handleSort("mode_id", sortState, setSortState, setSortedData, tableData)}>
                                        {`Mode`}
                                        {getArrowIcon("mode_id")}
                                    </th>
                                )}
                            ...
                            {sortedData && sortedData.map((row: any, index: any) => {
                                return (
                                    ...
                                    >
                                        ...
                                        {columnVisibility.mode && (
                                            <td className="px-2 py-1 text-[#464255]">{row?.mode?.mode && row?.mode?.mode}</td>
                                        )}
