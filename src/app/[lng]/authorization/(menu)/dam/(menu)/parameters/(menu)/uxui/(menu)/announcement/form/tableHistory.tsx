...
const TableAnnouncementHistory: React.FC<TableProps> = ({ tableData, isLoading, columnVisibility }) => {
                ...
                <thead className="text-xs text-[#ffffff] bg-[#1473A1] sticky top-0 z-1">
                    <tr className="h-9">
                        ...
                        {columnVisibility.status && (
                            <th scope="col" className={`${table_sort_header_style} text-center`} onClick={() => handleSort("status", sortState, setSortState, setSortedData, fillMissingUpdateByAccount(tableData))}>
                                {`Status`}
                                {getArrowIcon("status")}
                            </th>
                        )}
                        ...
                        {columnVisibility.detail && (
                            <th scope="col" className={`${table_sort_header_style}`} onClick={() => handleSort("detail", sortState, setSortState, setSortedData, fillMissingUpdateByAccount(tableData))}>
                                {`Detail`}
                                {getArrowIcon("detail")}
                            </th>
                        )}
...
export default TableAnnouncementHistory;