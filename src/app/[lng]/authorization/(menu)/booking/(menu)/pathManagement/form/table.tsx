...
import { formatDate, formatDateNoTime, formatDateTimeSec } from '@/utils/generalFormatter';
...
const TablePathMgn: React.FC<TableProps> = ({ openEditForm, openViewForm, openHistoryForm, openViewModal, tableData, isLoading, columnVisibility, userPermission }) => {
                    ...
                    <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                        ...
                        <tbody>
                            {sortedData && sortedData?.map((row: any, index: any) => {
                                // const isDisabled = new Date() > new Date(row.start_date);
                                return (
                                    <tr
                                        key={row?.id}
                                            ...
                                            <td className="px-2 py-1">
                                                ...
                                                <div className="relative inline-flex justify-center items-center w-full">
                                                    ...
                                                    {openPopoverId === row?.id && (
                                                        <div ref={popoverRef}
                                                            className="absolute left-[-5.5rem] top-[-10px] mt-2 w-36 bg-white border border-gray-300 rounded-lg shadow-lg z-50">
                                                            <ul className="py-2">
                                                                ...
                                                                {
                                                                    // if present date > row?.start_date ("2025-01-23T00:00:00.000Z") disable this <li></li>
                                                                    userPermission?.f_edit && new Date().setHours(0, 0, 0, 0) <= new Date(row?.start_date).setHours(0, 0, 0, 0) && (
                                                                        <li
                                                                ...
                                                                {/* {
                                                                    userPermission?.f_edit && (
                                                                        <li
                                                                            className={`px-4 py-2 font-bold text-sm text-[#58585A] hover:bg-gray-100 ${isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                                                                            onClick={!isDisabled ? () => toggleMenu("edit", row?.id) : undefined}
                                                                        >
