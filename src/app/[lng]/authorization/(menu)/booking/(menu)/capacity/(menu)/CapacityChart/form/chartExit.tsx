...
const ChartExit: React.FC<any> = ({ }) => {
    ...
    // const [srchEndDate, setSrchEndDate] = useState<Date | null>(new Date('Sat Jan 02 2026 00:00:00 GMT+0700 (Indochina Time)'));
    ...
    const [srchStartDateMain, setSrchStartDateMain] = useState<Date | null>(new Date('Mon Jan 04 2010 00:00:00 GMT+0700 (Indochina Time)'));
    ...
    const filteredDatasets = datasets ? datasets?.filter((dataset?: any) => dataset.data.length > 0) : [];
            ...
            <aside className="flex flex-wrap sm:flex-row gap-2 pb-2 w-full">
                ...
                <InputSearch
                    ...
                    options={areaMaster?.data?.filter((item: any) =>
                        item?.entry_exit?.id == 2)?.map((item: any) => ({ // filter เอาแค่ exit
