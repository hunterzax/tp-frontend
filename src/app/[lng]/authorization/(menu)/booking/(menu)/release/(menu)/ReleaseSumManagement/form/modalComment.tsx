...
const ModalComment: React.FC<FormExampleProps> = ({
    ...
    const handleSendComment = async () => {
        ...
        const res_comment = await postService(`/master/release-uioli-summary-management/comment`, data_post)
                    ...
                    <div className="flex flex-col items-center gap-2 p-9 w-[700px]">
                        <div className="w-full">
                            ...
                            <div className="mb-4 w-[100%]">
                                <div className="grid grid-cols-4 w-full text-sm font-semibold text-[#58585A]">
                                    <p>{`Contract Code`}</p>
                                    <p>{`Shipper Name`}</p>
                                    <p>{`Entry Point`}</p>
                                    <p>{`Exit Point`}</p>
                                ...
                                <div className="grid grid-cols-4 text-sm font-light text-[#58585A]">
                                    <p>{dataMain?.contract_code?.contract_code || '-'}</p>
                                    <p>{dataMain?.group?.name || '-'}</p>
                                    <p>{dataMain?.release_summary_detail?.[0]?.temp_contract_point || '-'}</p>
                                    <p>{dataMain?.release_summary_detail?.[1]?.temp_contract_point || '-'}</p>
                                </div>
