...
import { cutUploadFileName, formatDateTimeNoPlusSeven } from '@/utils/generalFormatter';
                        ...
                        <div className="w-full">
                            ...
                            <div className="mb-4 w-[80%]">
                                ...
                                <div className="grid grid-cols-3 text-sm font-light text-[#58585A]">
                                    ...
                                    <p>{data?.group?.name || ''}</p>
                                    <p>{data?.nomination_type?.document_type || ''}</p>
                                </div>
                            </div>
                        </div>

                        <div
                            className={`mb-4 w-[100%] ${data?.upload_template_for_shipper_file?.length > 3 ? 'max-h-[350px] overflow-y-auto' : ''}`}
                        >

                            {/* only lasted 1 file */}
                            {data?.upload_template_for_shipper_file?.[0] && (
                                <div key={data.upload_template_for_shipper_file[0].id} className="w-full h-auto mb-2 p-2 border rounded-lg">
                                    <div className="flex flex-col p-2">
                                        <div className="mb-2 flex justify-between">
                                            <span className='font-light'>
                                                By <span className="font-bold text-[#58585A]">
                                                    {data.upload_template_for_shipper_file[0]?.create_by_account?.first_name}{" "}
                                                    {data.upload_template_for_shipper_file[0]?.create_by_account?.last_name}
                                                </span>
                                            </span>
                                            <span className="text-[#9CA3AF] font-light">
                                                {formatDateTimeNoPlusSeven(data.upload_template_for_shipper_file[0]?.create_date)}
                                            </span>
                                        </div>
                                        <div className="flex justify-between items-center w-full h-[50px] border rounded-lg mb-2 p-4">
                                            <p className="flex items-center">
                                                ...
                                                {" "}
                                                {data.upload_template_for_shipper_file[0]?.url
                                                    ? cutUploadFileName(data.upload_template_for_shipper_file[0].url)
                                                    : 'no data'}
                                            </p>
                                            <span className="flex items-center">
                                                {data.upload_template_for_shipper_file[0]?.url && (
                                                    <a
                                                        href={data.upload_template_for_shipper_file[0].url}
                                                        target="_blank"
