...
const TableDailyArea: React.FC<any> = ({ tableData, isLoading, userPermission, zoneText, tempData, setTempData, tempDataConcept, setTempDataConcept, areaMaster, entryExitMaster, setIsEdited, tabEntry, tabConcept, tabIndex2ndTab }) => {
    ...
    const filterColumnsByTabIndex = (tabIndex: number) => {
        return initialColumnsTabEntryExit.filter((col: any) => {
            ...
            // const alwaysVisibleKeys = ["gas_day", "area", "total_cap", "nomination_point", "utilization"]
            const alwaysVisibleKeys = ["gas_day", "area", "total_cap", "nomination_point"]
    ...
    const handleSearch = (query: string) => {
        const queryLower = query ? query.replace(/\s+/g, '')?.toLowerCase().trim() : '';
        ...
        let filtered = dataTable.filter(
            (item: any) => {
                return (
                    ...
                    formatNumberThreeDecimalNom(item?.utilization)?.toString().replace(/\s+/g, '').toLowerCase().trim().includes(queryLower) ||
                    ...
                    formatNumberThreeDecimalNom(item?.imbalance_percent)?.toString().replace(/\s+/g, '').toLowerCase().trim().includes(queryLower) ||
                    ...
                    formatNumberThreeDecimalNom(item?.park)?.toString().replace(/\s+/g, '').toLowerCase().trim().includes(queryLower) ||
                    ...
                    formatNumberThreeDecimalNom(item?.unpark)?.toString().replace(/\s+/g, '').toLowerCase().trim().includes(queryLower) ||
                    ...
                    formatNumberThreeDecimalNom(item?.change_min_invent)?.toString().replace(/\s+/g, '').toLowerCase().trim().includes(queryLower) ||
                    ...
                    formatNumberThreeDecimalNom(item?.shrinkage)?.toString().replace(/\s+/g, '').toLowerCase().trim().includes(queryLower) ||
                ...
                <div className="flex items-center space-x-2 py-4 px-1">
                    ...
                    <div className="w-[100%] flex flex-wrap gap-2 justify-end">
                        ...
                        <BtnExport
                            ...
                            specificMenu='summary-nomination-report-daily-area'
                                ...
                                <thead className="text-xs text-[#ffffff] bg-[#1473A1] sticky top-0 z-10">
                                    <tr className="h-20">
                                        ...
                                        {columnVisibility.utilization && (
                                            // <th scope="col" className={`${table_sort_header_style}`} onClick={() => handleSort("utilization", sortState, setSortState, setSortedData, dataTable)}>
                                            <th scope="col" className={`${table_sort_header_style}`} onClick={() => handleSort("utilization", sortState, setSortState, setSortedData, paginatedData)}>
                                                {`Utilization (%)`}
                                        ...
                                        paginatedData?.length > 0 && paginatedData?.map((row: any, index: any) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {columnVisibility?.area && (
                                                        <td className={`px-2 py-1 ${row?.status ? "text-[#464255]" : "text-[#9CA3AF]"} !justify-center items-center text-center flex`}>
                                                            ...
                                                            {(() => {
                                                                ...
                                                                const filter_area = areaMaster?.data?.find((item: any) => item.name === (areaText && typeof areaText === 'string' ? areaText.trim() : ''));

                                                                return filter_area?.entry_exit_id == 2 ? (
                                                                    <div
                                                                        className="flex justify-center items-center rounded-full p-1 text-[#464255]"
                                                                        style={{ backgroundColor: filter_area?.color, width: '40px', height: '40px', color: filter_area?.color ? getContrastTextColor(filter_area.color) : '' }}
                                                                    >
                                                                        {`${filter_area?.name}`}
                                                                    </div>
                                                    ...
                                                    {/* {columnVisibility.nomination_point && (
                                                        <td className="px-2 py-1 text-[#464255] font-bold">{row?.nomination_point ? row?.nomination_point : ''}</td>
                                                    )} */}
