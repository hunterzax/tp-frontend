                    ...
                    <div className="flex flex-col items-center gap-2 p-9">
                        <div className="w-[600px]">
                            ...
                            <div className="mb-4 w-[65%]">
                                <div className="grid grid-cols-[120px_120px_220px] text-sm font-semibold text-[#58585A]">
                                    <p>{`Gas Day`}</p>
                                    <p>{`Time`}</p>
                                    <p>{`Daily Adjustment Code`}</p>
                                </div>

                                <div className="grid grid-cols-[120px_120px_220px] text-sm font-light text-[#58585A]">
                                    <p>{dataRow?.gas_day ? formatDateNoTime(dataRow?.gas_day) : ''}</p>
                                    <p>{dataRow?.time ? dataRow?.time : ''}</p>
                                    <p>{dataRow?.daily_code ? dataRow?.daily_code : ''}</p>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div className="mb-2 flex justify-between items-center">
                                            <div className="flex items-baseline gap-2">
                                                <span className='rounded-[20px] px-1 '>
                                                    <div className="flex min-w-[130px] max-w-[250px] w-auto text-center justify-center rounded-full p-1 text-[#464255]" style={{ backgroundColor: String(item?.daily_adjustment_status?.color) }}>{item?.daily_adjustment_status?.name}</div>
                                                </span>
