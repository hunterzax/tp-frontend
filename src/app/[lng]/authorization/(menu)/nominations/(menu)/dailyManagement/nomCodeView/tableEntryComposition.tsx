...
const TableEntryComposition: React.FC<any> = ({ tableData, isLoading, userPermission, zoneMaster, dataMasterZone }) => {
    ...
    useEffect(() => {
        if (tableData && tableData?.data_temp2?.valueData?.length > 0) {
            ...
            // setSortedData(tableData?.data_temp2?.valueData);
    ...
    return (<>
        {/* <div className={`relative h-[calc(100vh-370px)] overflow-y-auto block  rounded-t-md z-1`}> */}
                                        ...
                                        >
                                            ...
                                            {columnVisibility.co2 && (
                                                ...
                                                <td
                                                    className={(() => {
                                                        const val = row?.["2"];
                                                        const n = val == null ? null : Number(String(val).replace(/,/g, "").trim());

                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_carbon_dioxide_min;
                                                        const max = qm?.v2_carbon_dioxide_max;

                                                        const hasMin = Number.isFinite(min);
                                                        const hasMax = Number.isFinite(max);
                                                        const hasBounds = hasMin || hasMax; // ถ้าทั้งคู่ไม่มี → ไม่เช็ค

                                                        const outOfRange =
                                                            n != null &&
                                                            Number.isFinite(n) &&
                                                            hasBounds &&
                                                            ((hasMin && n < (min as number)) || (hasMax && n > (max as number)));

                                                        return `px-2 py-1 min-w-[120px] text-right ${outOfRange ? "text-[#ED1B24]" : "text-[#464255]"}`;
                                                    })()}
                                                >
                                                    {row?.["2"] ? formatNumberThreeDecimal(row?.["2"]) : ''}
                                                </td>
                                            )}
                                            ...
                                            {columnVisibility.c1 && (
                                                ...
                                                <td
                                                    className={(() => {
                                                        ...
                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_methane_min;
                                                        const max = qm?.v2_methane_max;
                                                ...
                                                >
                                                    {row?.["3"] ? formatNumberThreeDecimal(row?.["3"]) : ''}
                                                </td>
                                            )}

                                            {columnVisibility.c2 && (
                                                <td className={`px-2 py-1 min-w-[120px] text-right text-[#464255]`}>{row?.["4"] ? formatNumberThreeDecimal(row?.["4"]) : ''}</td>
                                            )}

                                            {columnVisibility.c3 && (
                                                <td className={`px-2 py-1 min-w-[120px] text-right text-[#464255]`}>{row?.["5"] ? formatNumberThreeDecimal(row?.["5"]) : ''}</td>
                                            )}

                                            {columnVisibility.ic4 && (
                                                <td className={`px-2 py-1 min-w-[120px] text-right text-[#464255]`}>{row?.["6"] ? formatNumberThreeDecimal(row?.["6"]) : ''}</td>
                                            )}

                                            {columnVisibility.nc4 && (
                                                <td className={`px-2 py-1 min-w-[120px] text-right text-[#464255]`}>{row?.["7"] ? formatNumberThreeDecimal(row?.["7"]) : ''}</td>
                                            )}
                                            ...
                                            {columnVisibility.c2_plus && (
                                                // <td className={`px-2 py-1 text-[#464255] min-w-[120px] text-right ${((parseFloat(row?.["12"]) < row?.zone?.zone_master_quality?.[0]?.v2_c2_plus_min || parseFloat(row?.["12"]) > row?.zone?.zone_master_quality?.[0]?.v2_c2_plus_max)) ? "text-[#ED1B24]" : "text-[#464255]"}`}>{row?.["12"] ? formatNumberThreeDecimal(row?.["12"]) : ''}</td>
                                                <td
                                                    className={(() => {
                                                        const val = row?.["12"];
                                                        const n = val == null ? null : Number(String(val).replace(/,/g, "").trim());
                                                        ...
                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_c2_plus_min;
                                                        const max = qm?.v2_c2_plus_max;
                                                ...
                                                >
                                                    {row?.["12"] ? formatNumberThreeDecimal(row?.["12"]) : ''}
                                                </td>
                                            )}

                                            {columnVisibility.n2 && (
                                                // <td className={`px-2 py-1 text-[#464255] min-w-[120px] text-right ${((parseFloat(row?.["13"]) < row?.zone?.zone_master_quality?.[0]?.v2_nitrogen_min || parseFloat(row?.["13"]) > row?.zone?.zone_master_quality?.[0]?.v2_nitrogen_max)) ? "text-[#ED1B24]" : "text-[#464255]"}`}>{row?.["13"] ? formatNumberThreeDecimal(row?.["13"]) : ''}</td>
                                                <td
                                                    className={(() => {
                                                        const val = row?.["13"];
                                                        const n = val == null ? null : Number(String(val).replace(/,/g, "").trim());
                                                        ...
                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_nitrogen_min;
                                                        const max = qm?.v2_nitrogen_max;
                                                ...
                                                >
                                                    {row?.["13"] ? formatNumberThreeDecimal(row?.["13"]) : ''}
                                                </td>
                                            )}

                                            {columnVisibility.o2 && (
                                                // <td className={`px-2 py-1 text-[#464255] min-w-[120px] text-right ${((parseFloat(row?.["14"]) < row?.zone?.zone_master_quality?.[0]?.v2_oxygen_min || parseFloat(row?.["14"]) > row?.zone?.zone_master_quality?.[0]?.v2_oxygen_max)) ? "text-[#ED1B24]" : "text-[#464255]"}`}>{row?.["14"] ? formatNumberThreeDecimal(row?.["14"]) : ''}</td>
                                                <td
                                                    className={(() => {
                                                        const val = row?.["14"];
                                                        const n = val == null ? null : Number(String(val).replace(/,/g, "").trim());
                                                        ...
                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_oxygen_min;
                                                        const max = qm?.v2_oxygen_max;
                                                ...
                                                >
                                                    {row?.["14"] ? formatNumberThreeDecimal(row?.["14"]) : ''}
                                                </td>
                                            )}

                                            {columnVisibility.h2s && (
                                                // <td className={`px-2 py-1 text-[#464255] min-w-[120px] text-right ${((parseFloat(row?.["15"]) < row?.zone?.zone_master_quality?.[0]?.v2_hydrogen_sulfide_min || parseFloat(row?.["15"]) > row?.zone?.zone_master_quality?.[0]?.v2_hydrogen_sulfide_max)) ? "text-[#ED1B24]" : "text-[#464255]"}`}>{row?.["15"] ? formatNumberThreeDecimal(row?.["15"]) : ''}</td>
                                                <td
                                                    className={(() => {
                                                        const val = row?.["15"];
                                                        const n = val == null ? null : Number(String(val).replace(/,/g, "").trim());
                                                        ...
                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_hydrogen_sulfide_min;
                                                        const max = qm?.v2_hydrogen_sulfide_max;
                                                ...
                                                >
                                                    {row?.["15"] ? formatNumberThreeDecimal(row?.["15"]) : ''}
                                                </td>
                                            )}

                                            {columnVisibility.s && (
                                                // <td className={`px-2 py-1 text-[#464255] min-w-[120px] text-right ${((parseFloat(row?.["16"]) < row?.zone?.zone_master_quality?.[0]?.v2_total_sulphur_min || parseFloat(row?.["16"]) > row?.zone?.zone_master_quality?.[0]?.v2_total_sulphur_max)) ? "text-[#ED1B24]" : "text-[#464255]"}`}>{row?.["16"] ? formatNumberThreeDecimal(row?.["16"]) : ''}</td>
                                                <td
                                                    className={(() => {
                                                        const val = row?.["16"];
                                                        const n = val == null ? null : Number(String(val).replace(/,/g, "").trim());
                                                        ...
                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_total_sulphur_min;
                                                        const max = qm?.v2_total_sulphur_max;
                                                ...
                                                >
                                                    {row?.["16"] ? formatNumberThreeDecimal(row?.["16"]) : ''}
                                                </td>
                                            )}

                                            {columnVisibility.hg && (
                                                // <td className={`px-2 py-1 text-[#464255] min-w-[120px] text-right ${((parseFloat(row?.["17"]) < row?.zone?.zone_master_quality?.[0]?.v2_mercury_min || parseFloat(row?.["17"]) > row?.zone?.zone_master_quality?.[0]?.v2_mercury_max)) ? "text-[#ED1B24]" : "text-[#464255]"}`}>{row?.["17"] ? formatNumberThreeDecimal(row?.["17"]) : ''}</td>
                                                <td
                                                    className={(() => {
                                                        const val = row?.["17"];
                                                        const n = val == null ? null : Number(String(val).replace(/,/g, "").trim());
                                                        ...
                                                        const qm = row?.zone?.zone_master_quality?.[0];
                                                        const min = qm?.v2_mercury_min;
                                                        const max = qm?.v2_mercury_max;
                                                ...
                                                >
                                                    {row?.["17"] ? formatNumberThreeDecimal(row?.["17"]) : ''}
                                                </td>
                                            )}
                                        </tr>
                                    )
