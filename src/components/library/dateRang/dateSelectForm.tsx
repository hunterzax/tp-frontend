...
import { formatDateBulletin, formatToUTC } from '@/utils/generalFormatter';
...
interface DatePickerSectionProps {
    ...
    readOnly?: boolean;
    valueShow?: string;
    termCondition?: any; // termCondition รับ selectedTermMaster[0]?.term_type_id มา
    bookingTemple?: any; // bookingTemple รับ term ที่ set ไว้ใน dam -> capa right template ตัวที่ active ปัจจุบัน
    termMinDate?: any;
    termMaxDate?: any;
    isStartDate?: boolean;
    isEndDate?: boolean;
...
const DatePickaForm = forwardRef<HTMLInputElement, DatePickerSectionProps>(({
    label,
    onChange,
    ...
    useEffect(() => {
        ...
        if (mode == 'edit-table') {
            if (valueShow) {
                ...
                const value_show = formatToUTC(valueShow);
                setSelectedDate(value_show);
            }
        }
        ...
        if (valueShow && valueShow.includes("/") && mode === 'edit') {
            const value_show = formatToUTC(valueShow)
            // const tomorrowDay = dayjs(valueShow, 'DD/MM/YYYY');
    ...
    const btnProps = "grid grid-cols-[90%_10%] w-full justify-center rounded-lg h-[44px] bg-white border pl-2 pr-2 border-[#DFE4EA]";
                ...
                {/* original code 2025-02-17 */}
                {
                    mode == 'edit-bulletin' || mode == 'edit-table' ?
                        ...
                        : mode == 'edit' || (readOnly && mode == 'edit') ?
                            ...
                            : mode == 'create' ?
                                <Typography fontSize={14} sx={{ textOverflow: "ellipsis", whiteSpace: "nowrap", overflow: "hidden", width: "90%" }} className={`${(valueShow && valueShow !== null && valueShow !== 'Invalid Date') || readOnly ? 'text-[#464255]' : 'text-[#9CA3AF]'} !text-[${splaceHolder}] placeholder-txt ${mode == 'view' && valueShow == "Invalid Date" ? 'opacity-0' : 'opacity-100'}`}>
    ...
    return (
        <DatePicker
            ...
            customInput={<CustomInput ref={ref} />} // ใช้ custom input และ forward ref
            ...
            filterDate={(date) => {
                ...
                // file_start_date_mode 1 = every day , 2 = fix day, 3 = to day+
                if (mode === 'edit-bulletin' && (Number(bookingTemple?.file_start_date_mode) === 2)) {
                    const day = date.getDate();
                    return day === Number(bookingTemple?.fixdayday);
                } else if (mode === 'edit-bulletin' && (Number(bookingTemple?.file_start_date_mode) === 1)) {
                    const today = new Date();
                    today.setHours(0, 0, 0, 0); // reset เวลา
                    return date > today; // ทุกวันตั้งแต่พรุ่งนี้เป็นต้นไป
                } else if (termCondition && (Number(termCondition) === 1 || Number(termCondition) === 2)) { // termCondition มันส่ง selectedTermMaster[0]?.term_type_id มา
...
DatePickaForm.displayName = 'DatePickaForm';
