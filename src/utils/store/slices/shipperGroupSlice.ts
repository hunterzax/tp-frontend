...
export const fetchShipperGroup = createAsyncThunk(
    'shippergroup/fetchShipperGroup',
    async (_, thunkAPI) => {
        try {
            ...
            const apiPath = '/master/asset/shipper-group';
...
const shipperGroupSlice = createSlice({
    name: 'shippergroup',
    initialState: {
        loading: false,
        data: null,
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchShipperGroup.pending, (state) => {
                state.loading = true;
            })
            .addCase(fetchShipperGroup.fulfilled, (state, action) => {
                state.loading = false;
                state.data = action.payload;
            })
            .addCase(fetchShipperGroup.rejected, (state: any, action) => {
                state.loading = false;
...
export default shipperGroupSlice.reducer;